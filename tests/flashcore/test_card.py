...
# Assuming the models are in cultivation.scripts.flashcore.card
# Adjust the import path based on your project structure and how pytest discovers modules.
# For this example, let's assume flashcore is installed or in PYTHONPATH.
try:
...
class TestReviewModel:
    ...
    def test_review_resp_ms_validation(self, valid_card_uuid):
        ...
        with pytest.raises(ValidationError):
            Review(card_uuid=valid_card_uuid, rating=0, resp_ms=-100, stab_after=1.0, diff=7.0,
                   next_due=date.today() + timedelta(days=1), elapsed_days_at_review=0,
                   scheduled_days_interval=1)
    ...
    def test_review_stab_after_validation(self, valid_card_uuid):
        ...
        with pytest.raises(ValidationError):
            Review(card_uuid=valid_card_uuid, rating=0, stab_after=0.05, diff=7.0,
                   next_due=date.today() + timedelta(days=1), elapsed_days_at_review=0,
                   scheduled_days_interval=1)
