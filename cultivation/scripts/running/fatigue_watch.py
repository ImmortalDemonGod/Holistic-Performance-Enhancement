...
# Removed unused imports: os, json, datetime as dt
...
def main():
    try:
        ...
        if len(df) < WINDOW:
            print("Not enough data yet, need at least", WINDOW, "days")
            return
        ...
        if (rest_delta > REST_HR_UP) or (hrv_drop > HRV_DROP_PCT) or srpe_hi or ankle_hi or hr_cap_breach:
            ...
            try:
                ...
                if result.returncode != 0:
                    print("⚠️  GitHub CLI not available. Cannot create issue.")
                    return

                run(
                    ["gh", "issue", "create", "-t", title, "-b", body, "-l", "fatigue-alert"],
                    check=True
                )
            except CalledProcessError as e:
                print("⚠️  Unable to create issue:", e)
            except FileNotFoundError:
                print("⚠️  GitHub CLI (gh) not found. Please install it to create issues.")
    except FileNotFoundError:
        print(f"Error: {SUBJ} not found")
        return
    except Exception as e:
        print(f"Error reading {SUBJ}: {e}")
        return
