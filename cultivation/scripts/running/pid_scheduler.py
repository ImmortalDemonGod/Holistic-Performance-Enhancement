...
def add_distance_and_speed(df):
    ...
    # After computing pace_min_per_km, also add pace_sec_km for downstream metrics
    if 'pace_min_per_km' in df.columns:
        df['pace_sec_km'] = df['pace_min_per_km'] * 60
    df['pace_min_per_mile'] = df['pace_min_per_km'] * 1.60934
    return df
...
def summarize_run(df, label):
    ...
    # --- Patch for metrics compatibility ---
    df_metrics = df.copy()
    col_map = {
        'heart_rate': 'hr',
        'distance_segment_m': 'dist',
        'time_delta_s': 'dt',
    }
    for src, tgt in col_map.items():
        if src in df_metrics.columns and tgt not in df_metrics.columns:
            df_metrics[tgt] = df_metrics[src]
    # Ensure pace_sec_km exists
    if 'pace_sec_km' not in df_metrics.columns and 'pace_min_per_km' in df_metrics.columns:
        df_metrics['pace_sec_km'] = df_metrics['pace_min_per_km'] * 60

    required_cols = ['hr', 'dist', 'dt', 'pace_sec_km']
    missing = [c for c in required_cols if c not in df_metrics.columns]
    if missing:
        print(f"[metrics] Cannot compute advanced metrics: missing columns {missing}")
    else:
        try:
            from metrics import run_metrics
            metrics = run_metrics(df_metrics, threshold_hr=175, resting_hr=50)
