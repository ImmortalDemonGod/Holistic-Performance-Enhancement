...
const PDFViewerApplication = {
  ...
  bindEvents() {
    ...
    // Listen for annotation editor state changes (highlight creation, etc.)
    eventBus._on("annotationeditorstateschanged", this.handleAnnotationEditorStatesChanged.bind(this), { signal });
  },

  /**
   * Handle annotation editor state changes (add/edit/delete), specifically for highlights.
   * @param {CustomEvent} event
   */
  async handleAnnotationEditorStatesChanged(event) {
    ...
    if (type === 'add' && value && AnnotationType && value.annotationType === AnnotationType.HIGHLIGHT) {
      ...
      // Placeholder for sending data to backend (uncomment and adapt as needed)
      /*
      try {
        const response = await fetch('/api/events', {
          ...
          body: JSON.stringify({
            ...
            data: highlightData,
            // Optionally add documentId, userId, etc.
          }),
        });
        if (!response.ok) {
          console.error('Failed to send highlight data to backend:', response.status, response.statusText);
        } else {
          console.log('Highlight data sent to backend successfully.');
        }
      } catch (error) {
        console.error('Error sending highlight data to backend:', error);
      }
      */
    }
  },
